...
* MainEmpty - Наступает когда очередь задачь пуста
...
`bool $flush` - если `true` то лог выполненых действий будет очищен после успешного выполнения целевого действия.
