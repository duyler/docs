...
* StateMainResume - наступает перед возвратом управления в действие. Если обработчики для состояния не определены и в `Fiber::suspend()` была передана callback-функция, она будет запущена немедленно, а возвращаемое callback-функцией значение будет резюмировано в контекст действия.
